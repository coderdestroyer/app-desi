...
<div class="space-y-6">
    ...
    <div class="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <a href="{{ route('operator.klassen.index') }}" class="ml-12 lg:ml-0 inline-flex items-center justify-center gap-2.5 px-4 py-2.5 rounded-xl bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 font-bold text-xs sm:text-sm transition-all shadow-xs w-full sm:w-auto">
    ...
    <div class="bg-white rounded-2xl border border-[#CFE3D5] shadow-xs p-4 sm:p-5 md:p-6">
        <div class="mb-5 sm:mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h2 class="text-lg sm:text-xl font-bold text-slate-800">Detail Laju Pertumbuhan & Kontribusi Per Sektor</h2>
                <p class="text-slate-500 text-xs mt-0.5">Laju Pertumbuhan Sektor (ri) vs Acuan (r) dan Share Sektor (yi) vs Acuan (y)</p>
            </div>
            <form action="{{ route('operator.klassen.show') }}" method="GET" class="relative w-full sm:w-80">
                ...
                @endif
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari Sektor atau Klasifikasi..." class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm focus:border-[#145239] focus:ring-1 focus:ring-[#145239] outline-none transition-all">
        ...
        <div class="w-full overflow-x-auto border border-slate-200/80 rounded-xl">
            <table id="klassenDetailTable" class="w-full min-w-[1000px] text-left border-collapse text-xs">
                ...
                <tbody class="divide-y divide-slate-100 font-medium text-slate-700">
                    @forelse($sectorData as $index => $data)
                        <tr class="hover:bg-emerald-50/30 transition-colors">
                            ...
                            <td class="px-4 py-3.5 text-center font-mono font-bold whitespace-nowrap">{{ number_format($data['ri'] ?? 0, 2, ',', '.') }}%</td>
                            <td class="px-4 py-3.5 text-center font-mono font-bold whitespace-nowrap">{{ number_format($data['r'] ?? 0, 2, ',', '.') }}%</td>
                            <td class="px-4 py-3.5 text-center font-mono font-bold whitespace-nowrap">{{ number_format($data['yi'] ?? 0, 2, ',', '.') }}%</td>
                            <td class="px-4 py-3.5 text-center font-mono font-bold whitespace-nowrap">{{ number_format($data['y'] ?? 0, 2, ',', '.') }}%</td>
