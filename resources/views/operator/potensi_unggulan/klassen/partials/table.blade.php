<div class="w-full overflow-x-auto border border-slate-200/80 rounded-xl">
    <table id="klassenTable" class="w-full min-w-[950px] text-left border-collapse text-xs">
        ...
        <tbody class="divide-y divide-slate-100 font-medium text-slate-700">
            @forelse($klassenData as $index => $data)
                <tr class="hover:bg-emerald-50/30 transition-colors {{ !empty($data['is_provinsi']) ? 'bg-emerald-50/40 font-semibold' : '' }}">
                    <td class="px-4 py-3.5 text-center font-mono text-slate-400">{{ ($klassenData->currentPage() - 1) * $klassenData->perPage() + $loop->iteration }}</td>
