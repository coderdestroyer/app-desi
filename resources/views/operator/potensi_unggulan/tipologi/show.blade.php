...
@section('title', 'Detail Sektor Tipologi Sektor - ' . $namaDaerah . ' (' . $tahun . ')')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <a href="{{ route('operator.tipologi.index') }}" class="ml-12 lg:ml-0 inline-flex items-center justify-center gap-2.5 px-4 py-2.5 rounded-xl bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 font-bold text-xs sm:text-sm transition-all shadow-xs w-full sm:w-auto">
    ...
    <div class="bg-white rounded-2xl border border-[#CFE3D5] shadow-xs p-4 sm:p-5 md:p-6">
        <div class="mb-5 sm:mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h2 class="text-lg sm:text-xl font-bold text-slate-800">Detail Matriks Tipologi Sektor Per Sektor</h2>
                <p class="text-slate-500 text-xs mt-0.5">Penggabungan Nilai LQ (Rasio Kontribusi) dan SS (Keunggulan Kompetitif / Cij)</p>
            </div>
            <form action="{{ route('operator.tipologi.show') }}" method="GET" class="relative w-full sm:w-80">
                ...
                @endif
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari Sektor atau Kuadran..." class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm focus:border-[#145239] focus:ring-1 focus:ring-[#145239] outline-none transition-all">
        ...
        <div class="w-full overflow-x-auto border border-slate-200/80 rounded-xl">
            <table id="tipologiDetailTable" class="w-full min-w-[1000px] text-left border-collapse text-xs">
                <thead class="bg-slate-50 border-b border-slate-200 text-slate-600 font-bold uppercase tracking-wider">
                    <tr>
                        ...
                        <th class="px-4 py-3.5 text-center whitespace-nowrap">NILAI SS (Cij)</th>
                        <th class="px-4 py-3.5 text-center whitespace-nowrap">KUADRAN</th>
                        <th class="px-4 py-3.5 min-w-[220px]">KLASIFIKASI TIPOLOGI</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 font-medium text-slate-700">
                    @forelse($sectorData as $index => $data)
                        <tr class="hover:bg-emerald-50/30 transition-colors">
                            ...
                            <td class="px-4 py-3.5 text-center font-mono font-bold whitespace-nowrap">{{ number_format($data['nilai_lq'] ?? 0, 2, ',', '.') }}</td>
                            <td class="px-4 py-3.5 text-center font-mono font-bold whitespace-nowrap">{{ number_format($data['nilai_ss'] ?? 0, 2, ',', '.') }}</td>
                            <td class="px-4 py-3.5 text-center whitespace-nowrap">
                                @if($data['kuadran'] === 'Kuadran I')
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-800 border border-emerald-200 whitespace-nowrap">
                                        Kuadran I
                                    </span>
                                @elseif($data['kuadran'] === 'Kuadran II')
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-amber-100 text-amber-800 border border-amber-200 whitespace-nowrap">
                                        Kuadran II
                                    </span>
                                @elseif($data['kuadran'] === 'Kuadran III')
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-orange-100 text-orange-800 border border-orange-200 whitespace-nowrap">
                                        Kuadran III
                                    </span>
                                @else
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-rose-100 text-rose-800 border border-rose-200 whitespace-nowrap">
                                        Kuadran IV
                                    </span>
                                @endif
                            </td>
                            <td class="px-4 py-3.5 text-slate-700 font-medium">{{ $data['tipologi'] ?? ($data['kategori_sektor'] ?? '-') }}</td>
                        </tr>
                    @empty
                        <tr>
...
<script>
    function exportToExcel() {
        ...
        var table = document.getElementById("tipologiDetailTable");
        if (!table) return;
        var clone = table.cloneNode(true);
        var wb = XLSX.utils.table_to_book(clone, {sheet: "Detail Tipologi Sektor"});
        XLSX.writeFile(wb, "Detail_Tipologi_Sektor_{{ Str::slug($namaDaerah) }}_{{ $tahun }}.xlsx");
    }
</script>
@endsection
