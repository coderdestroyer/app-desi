...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <a href="{{ route('operator.tipologi.index') }}" class="ml-12 lg:ml-0 inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 font-bold text-xs md:text-sm transition-all shadow-xs">
    ...
    <div class="bg-white rounded-2xl border border-[#CFE3D5] shadow-xs p-6">
        <div class="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                <h2 class="text-xl font-bold text-slate-800">Detail Matriks Tipologi Sektor Per Sektor</h2>
                <p class="text-slate-500 text-xs mt-0.5">Penggabungan Nilai LQ (Rasio Kontribusi) dan SS (Keunggulan Kompetitif / Cij)</p>
            </div>
            <form action="{{ route('operator.tipologi.show') }}" method="GET" class="relative w-full md:w-80">
                ...
                @endif
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari Sektor atau Kuadran..." class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:border-[#145239] focus:ring-1 focus:ring-[#145239] outline-none transition-all">
        ...
        <div class="overflow-x-auto border border-slate-200/80 rounded-xl">
            <table id="tipologiDetailTable" class="w-full text-left border-collapse min-w-[1000px]">
                ...
                <tbody class="divide-y divide-slate-100 text-sm text-slate-700">
                    @forelse($sectorData as $index => $data)
                        <tr class="hover:bg-emerald-50/30 transition-colors">
                            ...
                            <td class="px-4 py-3.5 text-center font-mono text-xs font-bold">{{ number_format($data['nilai_lq'] ?? 0, 2, ',', '.') }}</td>
                            <td class="px-4 py-3.5 text-center font-mono text-xs font-bold">{{ number_format($data['nilai_ss'] ?? 0, 2, ',', '.') }}</td>
