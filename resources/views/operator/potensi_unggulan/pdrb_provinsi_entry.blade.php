...
<div class="max-w-6xl mx-auto space-y-6" x-data="pdrbEntryData()">
    ...
    <div class="flex items-center justify-between">
        <a href="{{ route('operator.pdrb-provinsi.index') }}" class="ml-12 lg:ml-0 inline-flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-white border border-[#CFE3D5] text-sm font-bold text-[#145239] hover:bg-[#EEF8F2] transition-colors shadow-sm">
    ...
    {{-- Banner Header --}}
    <div class="bg-gradient-to-r from-[#145239] via-[#0F8A5F] to-[#1E5D41] rounded-2xl p-7 md:p-10 shadow-xl text-white relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-6">
    ...
    <form action="{{ route('operator.pdrb-provinsi.save-entry') }}" method="POST" class="bg-white rounded-2xl border border-[#CFE3D5] shadow-sm p-6 md:p-8 space-y-6">
        ...
        <div class="flex items-center justify-between border-b border-slate-100 pb-4">
            <div class="flex items-center gap-3 text-base md:text-lg font-bold text-slate-800">
                <div class="w-9 h-9 rounded-xl bg-[#EEF8F2] text-[#145239] flex items-center justify-center font-bold border border-[#CFE3D5]">
