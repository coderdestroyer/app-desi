...
@section('title', 'Detail Sektor Shift-Share - ' . $namaDaerah . ' (' . $tahunAwal . '-' . $tahunAkhir . ')')
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <a href="{{ route('operator.ss.index') }}" class="ml-12 lg:ml-0 inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 font-bold text-xs md:text-sm transition-all shadow-xs">
    ...
    <div class="bg-white rounded-2xl border border-[#CFE3D5] shadow-xs p-6">
        <div class="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                <h2 class="text-xl font-bold text-slate-800">Detail Komponen N, M, C & D Per Sektor</h2>
                <p class="text-slate-500 text-xs mt-0.5">Pertumbuhan Nasional (N), Bauran Proproporsional (M), Keunggulan Kompetitif (C), dan Net Change (D)</p>
            </div>
            <form action="{{ route('operator.ss.show') }}" method="GET" class="relative w-full md:w-80">
        ...
        <div class="overflow-x-auto border border-slate-200/80 rounded-xl">
            <table id="ssDetailTable" class="w-full text-left border-collapse min-w-[1000px]">
                <thead class="bg-slate-50 border-b border-slate-200 text-slate-600 text-xs font-bold uppercase tracking-wider">
                    <tr>
                        ...
                        <th class="px-4 py-3.5 text-right whitespace-nowrap">N (NASIONAL)</th>
                        <th class="px-4 py-3.5 text-right whitespace-nowrap">M (PROPORSIONAL)</th>
                        <th class="px-4 py-3.5 text-right whitespace-nowrap">C (KOMPETITIF)</th>
                        <th class="px-4 py-3.5 text-right whitespace-nowrap">D (TOTAL / DIJ)</th>
                        <th class="px-4 py-3.5 text-center whitespace-nowrap">PERTUMBUHAN</th>
                        <th class="px-4 py-3.5 text-center whitespace-nowrap">DAYA SAING</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 text-sm text-slate-700">
                    @forelse($sectorData as $index => $data)
                        <tr class="hover:bg-emerald-50/30 transition-colors">
                            ...
                            <td class="px-4 py-3.5 text-right font-mono text-xs">{{ number_format($data['nij'] ?? 0, 2, ',', '.') }}</td>
                            <td class="px-4 py-3.5 text-right font-mono text-xs">{{ number_format($data['mij'] ?? 0, 2, ',', '.') }}</td>
