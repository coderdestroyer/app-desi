...
<div class="max-w-7xl mx-auto space-y-6" x-data="{ isPdrbModalOpen: false }">
    ...
    <div class="bg-gradient-to-r from-[#145239] via-[#0F8A5F] to-[#1E5D41] rounded-2xl p-6 md:p-8 shadow-xl relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-6">
        <div class="relative z-10 text-white flex-1">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-800/50 border border-emerald-700/50 text-emerald-100 text-xs font-bold mb-3 backdrop-blur-sm">
                <i class="fa-solid fa-database text-[#FFD54F]"></i>
                <span>Pengelolaan Data Makroekonomi</span>
            </div>
            <h1 class="text-2xl md:text-3xl font-extrabold tracking-tight mb-2">
                Input & Kelola Data PDRB Daerah
            </h1>
            <p class="text-emerald-100/90 text-xs md:text-sm max-w-2xl leading-relaxed">
                Kelola basis data Produk Domestik Regional Bruto (PDRB) daerah sesuai otorisasi akun Anda. Setiap tahun & daerah dapat diisi dan diedit pada halaman khusus.
            </p>
        </div>
    ...
    <!-- Filter Bar -->
    <div class="bg-white p-5 rounded-2xl border border-[#CFE3D5] shadow-sm">
        <form action="{{ route('operator.pdrb.index') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
            <div>
                <label class="block font-semibold text-slate-700 mb-1">Filter Kabupaten / Kota</label>
                <select name="kabupaten_id" onchange="this.form.submit()" class="w-full rounded-xl border-[#CFE3D5] focus:border-[#145239] text-xs bg-white h-10">
            ...
            <div>
                ...
                <select name="tahun" onchange="this.form.submit()" class="w-full rounded-xl border-[#CFE3D5] focus:border-[#145239] text-xs bg-white h-10 font-mono">
            ...
            <div class="flex items-end gap-2">
                <button type="submit" class="w-full h-10 rounded-xl bg-[#145239] hover:bg-[#0B5D3D] text-white font-bold transition-colors flex items-center justify-center gap-1.5 shadow-sm">
                    <i class="fa-solid fa-filter"></i>
                    <span>Terapkan Filter</span>
                </button>
                @if(request()->anyFilled(['kabupaten_id', 'tahun']))
                    <a href="{{ route('operator.pdrb.index') }}" class="h-10 px-3 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 flex items-center justify-center transition-colors" title="Reset Filter">
    ...
    <div class="bg-white rounded-2xl border border-[#CFE3D5] shadow-sm overflow-hidden">
        <div class="p-5 border-b border-[#EEF8F2] bg-[#F8FAFC] flex items-center justify-between">
            <h3 class="font-bold text-slate-800 text-sm flex items-center gap-2">
                <i class="fa-solid fa-table-list text-[#145239]"></i>
                Daftar Tahun & Daerah PDRB Terdaftar
            </h3>
            <span class="text-xs font-semibold text-[#145239] bg-[#EEF8F2] px-3 py-1 rounded-full border border-[#CFE3D5]">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse text-xs">
                <thead>
                    <tr class="bg-[#EEF8F2] text-[#145239] font-bold border-b border-[#CFE3D5] uppercase tracking-wider">
