...
@section('title', 'Detail Nilai PDRB Provinsi ' . $provinsi->nama_provinsi . ' Tahun ' . $tahun)

@section('content')
<div class="max-w-6xl mx-auto space-y-6" x-data="pdrbProvinsiDetailData()">

    {{-- Breadcrumb & Action Bar --}}
    <div class="flex items-center justify-between">
        <a href="{{ route('operator.pdrb-provinsi.index', ['tab' => 'all']) }}" class="ml-12 lg:ml-0 inline-flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-white border border-[#CFE3D5] text-xs sm:text-sm font-bold text-[#145239] hover:bg-[#EEF8F2] transition-colors shadow-xs">
    ...
    <section class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-[#145239] via-[#0F8A5F] to-[#1E5D41] p-6 sm:p-7 md:p-10 shadow-xl text-white flex flex-col md:flex-row items-start md:items-center justify-between gap-5">
        <div class="space-y-2 sm:space-y-3 relative z-10">
            <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-sky-800/60 border border-sky-700/60 text-sky-100 text-xs font-bold backdrop-blur-sm">
                ...
                <span>Detail Nilai Sektor PDRB Provinsi (Read Only)</span>
            </div>
            <h1 class="text-2xl sm:text-3xl md:text-4xl font-extrabold tracking-tight">
                {{ $provinsi->nama_provinsi }} &bull; Tahun {{ $tahun }}
            </h1>
            <p class="text-emerald-100/90 text-xs sm:text-sm md:text-base max-w-2xl leading-relaxed">
                Melihat rincian nilai PDRB (dalam <strong>Rp Juta</strong>) untuk 17 Sektor Lapangan Usaha BPS di Provinsi ini.
            </p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-[#CFE3D5] shadow-xs p-5 sm:p-6 md:p-8 space-y-6">
        ...
        <div class="pt-6 border-t border-slate-100 flex items-center justify-start">
            <a href="{{ route('operator.pdrb-provinsi.index', ['tab' => 'all']) }}" class="w-full sm:w-auto px-6 py-3 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs sm:text-sm font-bold transition-colors flex justify-center items-center gap-2">
                <i class="fa-solid fa-arrow-left text-xs"></i>
                <span>Kembali ke Lihat PDRB Daerah</span>
            </a>
        </div>
...
<script>
function pdrbProvinsiDetailData() {
    return {
        grandTotal: '0,00',
        init() {
