            ...
            @if ($dataKbli->isNotEmpty())
                <footer class="flex flex-col gap-4 border-t border-slate-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                    <p class="m-0 text-xs text-slate-500">
                        Menampilkan <strong class="text-slate-700">{{ $paginator->firstItem() }}</strong>–<strong class="text-slate-700">{{ $paginator->lastItem() }}</strong> dari <strong class="text-slate-700">{{ number_format($paginator->total(), 0, ',', '.') }}</strong> {{ $hierarchyMode ? 'kategori' : 'data' }}
                    </p>
