...
@section('content')
    ...
    <div class="min-h-screen bg-slate-50 p-5 md:p-7 lg:p-8">
        ...
        <section
            ...
            <div class="relative z-10">
                <button type="button" onclick="window.location.href='{{ route('admin.pengguna.index', array_merge(
                    request()->except(['edit']),
                    ['mode' => 'create']
                )) }}'"
        ...
        <section class="
                    ...
                    shadow-sm
                ">
            <form action="{{ route('admin.pengguna.index') }}" method="GET" class="
                        grid
                        grid-cols-1
                        gap-4
                        md:grid-cols-2
                ...
                @if ($hasRoleColumn)
                    <div class="relative">
                        <select name="role" class="
                            ...
                            <option value="operator" @selected(request('role') === 'operator')>
                                Operator
                            </option>
                ...
                <div class="relative">
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Cari nama, email, role, atau status..." class="
                                h-11
                                w-full
                                rounded-xl
                                border
                    ...
                    <button type="submit" aria-label="Cari pengguna" class="
                                absolute
                                right-1.5
                                top-1/2
                                flex
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('admin.pengguna.index') }}" title="Reset filter" class="
                                inline-flex
                                h-11
                                w-11
                                flex-shrink-0
                        ...
                        @forelse ($pengguna as $item)
                                ...
                                STR_PAD_LEFT
                                                ...
                                                <td class="px-5 py-4">
                                                    ...
                                                    @if ($role === 'admin')
                                                        <span
                                                            class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold border border-amber-300 bg-amber-50 text-amber-800">
                                                            <i class="fa-solid fa-earth-asia text-[10px]"></i> Global (Semua Wilayah)
                                                        </span>
                                                    @elseif ($scope)
                                                        @if ($scope->kabupaten)
                                                            <span
                                                                class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold border {{ $isPending ? 'border-amber-300 bg-amber-50 text-amber-800' : 'border-sky-300 bg-sky-50 text-sky-800' }}"
                                                                title="{{ $scope->kabupaten->nama_kabupaten }}">
                                                                <i
                                                                    class="fa-solid {{ $isPending ? 'fa-hourglass-half' : 'fa-location-dot' }} text-[10px]"></i>
                                                                {{ $isPending ? 'Pengajuan' : 'Scope' }} Kab: {{ $scope->kabupaten->nama_kabupaten }}
                                                            </span>
                                                        @elseif ($scope->provinsi)
                                                            <span
                                                                class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold border {{ $isPending ? 'border-amber-300 bg-amber-50 text-amber-800' : 'border-emerald-300 bg-emerald-50 text-emerald-800' }}"
                                                                title="{{ $scope->provinsi->nama_provinsi }}">
                                                                <i class="fa-solid {{ $isPending ? 'fa-hourglass-half' : 'fa-map' }} text-[10px]"></i>
                                                                {{ $isPending ? 'Pengajuan' : 'Scope' }} Prov: {{ $scope->provinsi->nama_provinsi }} (+
                                                                Sub-Kab)
                                                            </span>
                                                        @else
                                                            <span
                                                                class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium border border-slate-200 bg-slate-100 text-slate-500">
                                                                Belum Set
                                                            </span>
                                                        @endif
                                                    @else
                                ...
                                array_merge(
                                    ...
                                    [
                                        ...
                                        'mode' => 'edit',
                                                        ...
                                                        <button type="button" title="Hapus pengguna" data-delete-user data-delete-url="{{ route(
                                'admin.pengguna.destroy',
                                $item->id
                            ) }}" data-delete-name="{{ $item->name }}" class="
                                                                        inline-flex
                                                                        h-9
                                                                        w-9
                                                                        items-center
        ...
        @if ($pengguna->hasPages())
            <section class="
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    @else
                        <a href="{{ $pengguna->previousPageUrl() }}" class="
                                            inline-flex
                                            h-9
                                            items-center
                                            gap-2
                    ...
                    @if ($pengguna->hasMorePages())
                        <a href="{{ $pengguna->nextPageUrl() }}" class="
                                            inline-flex
                                            h-9
                                            items-center
                                            gap-2
    ...
    @if ($isModalOpen)
        <div id="userFormModal" class="
            ...
            <div class="
                ...
                $isEdit ? $editData->name : ''
            ) }}" required autocomplete="name" placeholder="Masukkan nama pengguna" class="
                                            h-11
                                            w-full
                                            rounded-xl
                                            border
                        ...
                        <div>
                            ...
                            <input id="email" type="email" name="email" value="{{ old(
                'email',
                $isEdit ? $editData->email : ''
            ) }}" required autocomplete="email" placeholder="nama@email.com" class="
                                            h-11
                                            w-full
                                            rounded-xl
                                            border
                        ...
                        @if ($hasRoleColumn)
                            <div>
                                ...
                                <select id="role" name="role" x-model="role" required class="
                                                    h-11
                                                    w-full
                                                    rounded-xl
                                                    border
                                    ...
                                    <option value="operator" @selected($selectedRole === 'operator')>
                                        Operator
                                    </option>
                                </select>
                        ...
                        <div x-show="role === 'operator'"
                            ...
                            @if ($editScope)
                                <div
                                    class="p-3 rounded-xl bg-amber-50 border border-amber-200/80 text-amber-900 text-xs flex items-center justify-between">
                ...
                : 'Minimal 8 karakter'
                                ...
                                <button type="button" data-toggle-password="password" class="
                                                absolute
                                                right-3
                                                top-1/2
                                                -translate-y-1/2
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input id="password_confirmation" type="password" name="password_confirmation"
                                    autocomplete="new-password" placeholder="Ulangi password" @required(!$isEdit) class="
                                                h-11
                                                w-full
                                                rounded-xl
                                                border
                                ...
                                <button type="button" data-toggle-password="password_confirmation" class="
                                                absolute
                                                right-3
                                                top-1/2
                                                -translate-y-1/2
    ...
    <div id="deleteUserModal" class="
        ...
        <div class="
            ...
            <p class="
                    ...
                    ">
                Pengguna
                <strong id="deleteUserName" class="text-slate-700"></strong>
                akan dihapus dari sistem. Tindakan ini tidak dapat dibatalkan.
            </p>

            <form id="deleteUserForm" action="" method="POST" class="
                        mt-6
                        flex
                        flex-col-reverse
                        gap-3
                ...
                <button type="button" id="cancelDeleteUser" class="
                            inline-flex
                            h-11
                            min-w-32
                            items-center
