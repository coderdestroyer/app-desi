...
@section('content')
<div class="min-h-screen bg-slate-50 p-5 md:p-7 lg:p-8 space-y-6" x-data="pdrbEntryData()">

    {{-- Breadcrumb & Action Bar --}}
    <div class="flex items-center justify-between">
        <a href="{{ route('admin.pdrb.index') }}" class="inline-flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-white border border-slate-200 text-sm font-bold text-[#145239] hover:bg-[#EEF8F2] transition-colors shadow-xs">
    ...
    <section class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-[#145239] via-[#0F8A5F] to-[#1E5D41] p-7 md:p-8 shadow-lg text-white flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div class="bg-white/15 border border-white/20 backdrop-blur-md p-5 rounded-2xl text-right shrink-0 relative z-10 shadow-inner">
            <span class="block text-xs uppercase tracking-wider text-emerald-200 font-bold mb-1">Estimasi Total PDRB</span>
    ...
    {{-- Main Form Card --}}
    <form action="{{ route('admin.pdrb.save-entry') }}" method="POST" class="bg-white rounded-2xl border border-slate-200 shadow-xs p-6 md:p-8 space-y-6">
        @csrf
        ...
        <div class="flex items-center justify-between border-b border-slate-100 pb-4">
            <div class="flex items-center gap-3 text-base md:text-lg font-bold text-slate-800">
                ...
                <span>Isi Nilai PDRB Menurut 17 Sektor Lapangan Usaha Default (BPS)</span>
            </div>
        ...
        <div class="pt-6 border-t border-slate-100 flex items-center justify-between">
            <a href="{{ route('admin.pdrb.index') }}" class="px-6 py-3 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-bold transition-colors">
                Batal / Kembali
            </a>
