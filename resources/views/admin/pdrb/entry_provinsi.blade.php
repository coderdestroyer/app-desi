...
<div class="min-h-screen bg-slate-50 p-4 sm:p-6 md:p-7 lg:p-8 space-y-6" x-data="pdrbProvinsiEntryData()">
    ...
    <div class="flex items-center justify-between">
        <a href="{{ route('admin.pdrb.index', ['type' => 'provinsi']) }}" class="inline-flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-white border border-slate-200 text-xs sm:text-sm font-bold text-[#145239] hover:bg-[#EEF8F2] transition-colors shadow-xs">
    ...
    {{-- Main Form Card --}}
    <form action="{{ route('admin.pdrb.save-provinsi-entry') }}" method="POST" class="bg-white rounded-2xl border border-slate-200 shadow-xs p-5 sm:p-6 md:p-8 space-y-6">
        @csrf
        ...
        <div class="pt-6 border-t border-slate-100 flex flex-col-reverse sm:flex-row items-center justify-between gap-3">
            <a href="{{ route('admin.pdrb.index', ['type' => 'provinsi']) }}" class="w-full sm:w-auto px-6 py-3 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs sm:text-sm font-bold transition-colors flex justify-center items-center">
                Batal / Kembali
            </a>
