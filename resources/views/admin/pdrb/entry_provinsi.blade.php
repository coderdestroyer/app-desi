...
@section('content')
<div class="min-h-screen bg-slate-50 p-5 md:p-7 lg:p-8 space-y-6" x-data="pdrbProvinsiEntryData()">

    {{-- Breadcrumb & Action Bar --}}
    <div class="flex items-center justify-between">
        <a href="{{ route('admin.pdrb.index', ['type' => 'provinsi']) }}" class="inline-flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-white border border-slate-200 text-sm font-bold text-[#145239] hover:bg-[#EEF8F2] transition-colors shadow-xs">
    ...
    <section class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-[#145239] via-[#0F8A5F] to-[#1E5D41] p-7 md:p-8 shadow-lg text-white flex flex-col md:flex-row items-center justify-between gap-6">
        <div class="space-y-3 relative z-10">
            <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-800/60 border border-emerald-700/60 text-emerald-100 text-xs font-bold backdrop-blur-sm">
                ...
                <span>Admin Input & Edit Mode — PDRB Provinsi</span>
            </div>
            ...
            <p class="text-emerald-100/90 text-sm md:text-base max-w-2xl leading-relaxed">
                Silakan isikan atau perbarui nilai PDRB Provinsi (dalam <strong>Rp Juta</strong>) untuk 17 Sektor Lapangan Usaha BPS di bawah ini.
            </p>
        </div>
    ...
    {{-- Main Form Card --}}
    <form action="{{ route('admin.pdrb.save-provinsi-entry') }}" method="POST" class="bg-white rounded-2xl border border-slate-200 shadow-xs p-6 md:p-8 space-y-6">
        @csrf
        ...
        <div class="flex items-center justify-between border-b border-slate-100 pb-4">
            <div class="flex items-center gap-3 text-base md:text-lg font-bold text-slate-800">
                ...
                <span>Isi Nilai PDRB Provinsi 17 Sektor BPS</span>
            </div>
        ...
        <div class="pt-6 border-t border-slate-100 flex items-center justify-between">
            <a href="{{ route('admin.pdrb.index', ['type' => 'provinsi']) }}" class="px-6 py-3 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-bold transition-colors">
                Batal / Kembali
            </a>
            <button type="submit" class="px-8 py-3.5 rounded-xl bg-[#145239] hover:bg-[#0B5D3D] text-white text-sm font-extrabold shadow-lg hover:shadow-xl transition-all duration-300 flex items-center gap-2.5 transform hover:-translate-y-0.5">
                ...
                <span>Simpan Data PDRB Provinsi ({{ $tahun }})</span>
            </button>
        </div>
    </form>
</div>
