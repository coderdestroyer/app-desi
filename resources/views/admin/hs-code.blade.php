...
@section('content')
    ...
    <div class="min-h-screen bg-slate-50 p-4 sm:p-6 md:p-7 lg:p-8 space-y-6" x-data="{
        ...
        <section
            class="relative overflow-hidden rounded-2xl bg-gradient-to-r from-[#145239] via-[#0F8A5F] to-[#1E5D41] p-6 sm:p-7 md:p-8 shadow-lg text-white flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5">
        ...
        @elseif (!$columnsReady)
            <div
                class="mt-5 flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-medium text-red-700">
                <i class="fa-solid fa-circle-exclamation mt-0.5"></i>
                <span>Kolom tabel <strong>hs_codes</strong> belum sesuai. Minimal harus ada kolom <strong>hs_code</strong> dan
                    <strong>uraian_barang</strong>.</span>
            </div>
        @endif
        ...
        <section class="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm">
            <form action="{{ route('admin.hs-code.index') }}" method="GET" data-live-filter data-no-loader
                ...
                {{-- Filter Kategori --}}
                <div class="relative min-w-0 sm:col-span-1 {{ $hasStatusColumn ? 'xl:col-span-2' : 'xl:col-span-3' }}">
                ...
                {{-- Filter Subkelompok --}}
                <div class="relative min-w-0 sm:col-span-1 {{ $hasStatusColumn ? 'xl:col-span-2' : 'xl:col-span-2' }}">
                    <select name="subkelompok"
                        class="h-11 w-full appearance-none rounded-xl border border-slate-200 bg-white px-4 pr-10 text-sm font-medium text-slate-600 outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100">
                ...
                <div class="relative min-w-0 sm:col-span-1 xl:col-span-3">
                    ...
                    <button type="submit" aria-label="Cari HS Code"
                ...
                <div class="flex min-w-0 items-center justify-end sm:col-span-2 xl:col-span-1">
                    <a href="{{ route('admin.hs-code.index') }}" title="Reset filter"
        ...
        <div id="tableContainer" class="transition-opacity duration-200">
            <section id="adminHsTableCard"
                ...
                <header
                    class="flex flex-col justify-between gap-3 border-b border-slate-100 bg-slate-50/50 p-4 sm:p-5 sm:flex-row sm:items-center">
                    <div>
                ...
                <div id="adminHsTableWrapper" class="w-full overflow-x-auto">
                    <table id="adminHsTable" class="w-full min-w-[1000px] border-collapse text-left [&_tr[hidden]]:!hidden">
                        <thead>
                            <tr
                        ...
                        <tbody class="divide-y divide-slate-100 text-sm">
                            @forelse ($dataHsCode as $item)
                                ...
                                <tr class="transition-colors hover:bg-slate-50/60">
                                    ...
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        <span
                                            class="inline-flex items-center gap-1.5 rounded-xl border px-2.5 py-1 text-xs font-semibold {{ $statusLower === 'nonaktif' ? 'border-red-200 bg-red-50 text-red-700' : 'border-emerald-200 bg-emerald-50 text-emerald-700' }}">
                                            <span
                                    ...
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        @if ($hasIdColumn && $item->id)
                                            <div class="flex items-center justify-center gap-2">
                                                <a href="{{ route('admin.hs-code.index', array_merge(request()->query(), ['edit' => $item->id, 'mode' => 'edit'])) }}"
                                                    title="Edit HS Code"
                                                ...
                                                <button type="button" title="Hapus HS Code" data-delete-hs
                                                    data-delete-url="{{ route('admin.hs-code.destroy', $item->id) }}"
        ...
        <template x-teleport="body">
            <div x-show="isModalOpen" x-cloak x-transition @keydown.escape.window="closeModal()"
                ...
                <div class="bg-white rounded-2xl max-w-3xl w-full p-5 sm:p-6 shadow-2xl border border-emerald-100 relative my-auto max-h-[90vh] flex flex-col overflow-hidden"
                    @click.outside="closeModal()">
                    <div class="flex items-center justify-between pb-4 border-b border-slate-100 shrink-0">
                        <div class="flex items-center gap-3">
                            <div
                                class="w-10 h-10 rounded-xl bg-[#E7F2EB] text-[#145239] flex items-center justify-center font-bold shrink-0">
                                <i class="fa-solid" :class="isEdit ? 'fa-pen-to-square' : 'fa-plus'"></i>
                            </div>
                            <div>
                                <h3 class="font-bold text-slate-900 text-base"
                                    x-text="isEdit ? 'Edit Data HS Code' : 'Tambah Data HS Code'"></h3>
                                <p class="text-xs text-slate-500">Lengkapi kategori, kelompok, subkelompok, HS Code, serta
                                    uraian barang.</p>
                            </div>
                        </div>
                    ...
                    <form :action="formAction" method="POST"
                        class="overflow-y-auto pt-4 flex-1 space-y-4 text-sm text-slate-700">
                        @csrf
    ...
    <div id="deleteHsModal"
        ...
        <div class="w-full max-w-md rounded-2xl border border-slate-100 bg-white p-6 text-center shadow-2xl">
            ...
            <p class="mb-0 mt-2 text-sm leading-relaxed text-slate-500">
                Data HS Code <strong id="deleteHsName" class="text-slate-700"></strong> akan dihapus. Tindakan ini tidak
                dapat dibatalkan.
            </p>
